<template>
  <div class="m-t-40">
    <!-- 回款中 -->
    <el-row class="m-b-40">
      <el-tag>自动还款确认是否开启</el-tag>
      <el-button v-if="dictValue=='true'" @click="compensatorySubject">已开启</el-button>
      <el-button v-else @click="compensatorySubject">未开启</el-button>
      <el-button v-if="DataRoles($store.state.login.roles,'subjectsTab5btn1')" @click="autoSettleOn">开启自动回款</el-button>
      <el-button v-if="DataRoles($store.state.login.roles,'subjectsTab5btn2')" @click="manualSettle" type="success">申请回款</el-button>
    ...
    <el-table fit
      ...
      <el-table-column label="操作" width="200px;">
        <template slot-scope="scope">
          <a v-if="DataRoles($store.state.login.roles,'subjectsTab5btn8')" target="_blank"
             :href="scope.row.uri+'?view=pdf&param=Y'">查看合同</a>
          <router-link v-if="DataRoles($store.state.login.roles,'subjectsTab5btn9')"
                       :to="{path:'/admins/allassetslist/details', query: {id: scope.row.subjectId, userId: scope.row.userId}}">
            详情
          </router-link>
          <a v-if="DataRoles($store.state.login.roles,'subjectsTab5btn10')" @click="edit(scope.row)">编辑</a>
          <br>
          <a v-if="DataRoles($store.state.login.roles,'subjectsTab5btn11')"
             @click="showPlan(scope.row.subjectId)">还款计划</a>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCompensatorySubjectSwitch(){
        this.$axios({
          url: '/admin/api/subjects/getCompensatorySubjectSwitch',
          method: 'get'
        }).then(res => {
          this.dictValue = res.data.agreedSubject.dictValue;
      ...
      compensatorySubject(){
        const tag = this.dictValue ? '关闭' : '开启'
        this.$confirm(`是否${tag}，请确认!`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$axios({
            method: "GET",
            url: '/admin/api/subjects/updateAgreedSubject',
            params: {
              value: this.dictValue=='true'?'false':'true',
              oldValue: this.dictValue,
            }
          }).then(() => {
            console.log(`自动代偿${tag}成功`);
            this.getCompensatorySubjectSwitch()
          })
        })
      },
      autoSettleOn(){
        this.$confirm('确定将所有标的开启自动回款', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          this.$axios({
            method: "PUT",
            url: '/biz-params/auto.settlement',
            params: {
              value: true
            },
            header:{
