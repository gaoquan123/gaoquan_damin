         ...
         <el-table  fit
            ...
            <el-table-column  label="操作" width="200px;" >
                <template slot-scope="scope">
                    <a  target="_blank" :href="scope.row.uri+'?view=pdf'" v-if="DataRoles($store.state.login.roles,'subjectsTab10btn8')">查看合同</a>
                    <router-link :to="{path:'/admin/allassetslist/details', query: {id: scope.row.subjectId, userId: scope.row.userId}}"  v-if="DataRoles($store.state.login.roles,'subjectsTab10btn9')">详情</router-link>
